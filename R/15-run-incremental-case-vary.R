...
simulate_main <- function(generator, pars, samples, modeltype) {
  ...
  igra       <- c("C-TB")
  ptbld_vals <- c(1)
  c_ctb      <- seq(8,24,2)#16.9#+5.990338# 
...
get_icer<-function(par, sims, plot_switch = 1){
  ...
  icer_range <- quantile(icer[t_hor,], probs = c(0.025, 0.975), names = FALSE, na.rm=TRUE)
...
if(nsamples==1){
  ...
  if(model_parameters$model_pars$t_hor==5){
    ...
    file_name<-paste0("all_results_single_incremental_case_vary.csv")
    write.csv( results_mat,file.path(out_folder, file_name), row.names = FALSE)
    
    file_name<-paste0("summary_results_incremental_case_vary.csv")
    write.csv( results_summary,file.path(out_folder, file_name), row.names = FALSE)
  }
  ...
  file_name<-paste0("all_results_incremental_case_vary.csv")
  write.csv( results_mat,file.path(out_folder, file_name), row.names = FALSE)
  ...
  file_name<-paste0("all_icer_incremental_case_vary.csv")
  ...
  file_name<-paste0("all_inb_incremental_case_vary.csv")
  write.csv( icer_mat,file.path(out_folder, file_name), row.names = FALSE)
  ...
  file_name<-paste0("all_cost_incremental_case_vary.csv")
  ...
  file_name<-paste0("all_qaly_incremental_case_vary.csv")
